...
def clean_outliners(df):
    df_result = df.iloc[:0, :].copy()
    for lable in [lable for lable in df.columns if 'label_' in lable]:
        one_lable = df[df[lable] == 1.0]
        for col in [col for col in df.columns if 'label_' not in col and 'time' not in col]:
            tmparray = one_lable[col].values
            mean = one_lable[col].values.mean()
            for i in range(one_lable.shape[0]):
                if tmparray[i]:
                    one_lable.at[i,col] = mean
        pd.concat([df_result, one_lable])
    return df_result
